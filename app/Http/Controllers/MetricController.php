...
class MetricController extends Controller
{
    ...
    public function getMetricVIF($trafoId, $date)
    {
        ...
        $latestVoltageR =
            optional($voltages->sortByDesc("created_at")->first())->voltage_r ??
            0;
        $latestVoltageS =
            optional($voltages->sortByDesc("created_at")->first())->voltage_s ??
            0;
        $latestVoltageT =
            optional($voltages->sortByDesc("created_at")->first())->voltage_t ??
            0;
        ...
        $latestCurrentT =
            ...
            0;
        $latestCurrentIN =
            optional($currents->sortByDesc("created_at")->first())
                ->current_in ?? 0;

        $latestFrequency =
            optional($frequencies->sortByDesc("created_at")->first())
                ->frequency_r ?? 0;
    ...
    public function getMetricPQSPF($trafoId, $date)
    {
        ...
        $latestPowerR =
            optional($powers->sortByDesc("created_at")->first())->power_r ?? 0;
        $latestPowerS =
            optional($powers->sortByDesc("created_at")->first())->power_s ?? 0;
        $latestPowerT =
            optional($powers->sortByDesc("created_at")->first())->power_t ?? 0;

        $latestReactivePowerR =
            optional($reactivePowers->sortByDesc("created_at")->first())
                ->reactive_power_r ?? 0;
        $latestReactivePowerS =
            optional($reactivePowers->sortByDesc("created_at")->first())
                ->reactive_power_s ?? 0;
        $latestReactivePowerT =
            optional($reactivePowers->sortByDesc("created_at")->first())
                ->reactive_power_t ?? 0;

        $latestApparentPowerR =
            optional($apparentPowers->sortByDesc("created_at")->first())
                ->apparent_power_r ?? 0;
        $latestApparentPowerS =
            optional($apparentPowers->sortByDesc("created_at")->first())
                ->apparent_power_s ?? 0;
        $latestApparentPowerT =
            optional($apparentPowers->sortByDesc("created_at")->first())
                ->apparent_power_t ?? 0;

        $latestPowerFactorR =
            optional($powerFactors->sortByDesc("created_at")->first())
                ->power_factor_r ?? 0;
        $latestPowerFactorS =
            optional($powerFactors->sortByDesc("created_at")->first())
                ->power_factor_s ?? 0;
    ...
    public function getMetricTPO($trafoId, $date)
    {
        ...
        $latestPressure =
            ...
            0;
        $latestOilLevel =
            optional($oilLevels->sortByDesc("created_at")->first())
                ->oil_level ?? 0;
        $latestAmbientTemperature =
    ...
    public function getMetricAnalysis($trafoId, $date)
    {
        ...
        $powerLossR =
            ($latestCurrentR ** 2 * $latestVoltageR) / $latestCurrentR;
        $powerLossS =
            ($latestCurrentS ** 2 * $latestVoltageS) / $latestCurrentS;
        $powerLossT =
            ($latestCurrentT ** 2 * $latestVoltageT) / $latestCurrentT;

        // Resistive Voltage Drop
        $resistiveVoltageDropR =
            ($latestCurrentR * $latestVoltageR) / $latestCurrentR;
        $resistiveVoltageDropS =
            ($latestCurrentS * $latestVoltageS) / $latestCurrentS;
        $resistiveVoltageDropT =
            ($latestCurrentT * $latestVoltageT) / $latestCurrentT;
        ...
        $totalVoltageDropR =
            $latestCurrentR *
            sqrt($resistiveVoltageDropR ** 2 + $reactiveVoltageDropR ** 2);
        $totalVoltageDropS =
            $latestCurrentS *
            sqrt($resistiveVoltageDropS ** 2 + $reactiveVoltageDropS ** 2);
        $totalVoltageDropT =
            $latestCurrentT *
            sqrt($resistiveVoltageDropT ** 2 + $reactiveVoltageDropT ** 2);
    ...
    public function exportExcel($trafoId, $date)
    {
        ...
        return Excel::download(
            new MetricExport($trafoId, $date),
            "metrics_trafo_{$trafoId}_{$date}.xlsx"
        );
    }
}
