...
export default function ChartVIF({
    ...
    const metricAvgFrequency = singleLineChartString({
        labels: frequencies.map((frequency) =>
            timeMinuteString(new Date(frequency.created_at)),
        ),
                            ...
                            <Container sx={{ p: 2 }}>
                                <AggregationRST
                                    ...
                                    maxRTime={timeMinuteString(
                                        new Date(voltageRMetrics.timeOfMax),
                                    )}
                                    maxSTime={timeMinuteString(
                                        new Date(voltageSMetrics.timeOfMax),
                                    )}
                                    maxTTime={timeMinuteString(
                                        new Date(voltageTMetrics.timeOfMax),
                                    )}
                                    minRTime={timeMinuteString(
                                        new Date(voltageRMetrics.timeOfMin),
                                    )}
                                    minSTime={timeMinuteString(
                                        new Date(voltageSMetrics.timeOfMin),
                                    )}
                                    minTTime={timeMinuteString(
                                        new Date(voltageTMetrics.timeOfMin),
                                    )}
                                />
                            </Container>
                        </Box>
                            ...
                            <Container sx={{ p: 2 }}>
                                <AggregationSingle
                                    ...
                                    maxTime={timeMinuteString(
                                        new Date(frequencyMetrics.timeOfMax),
                                    )}
                                    minTime={timeMinuteString(
                                        new Date(frequencyMetrics.timeOfMin),
                                    )}
                                    latest={frequency_r}
                                />
                            </Container>
                        ...
                        >
                            ...
                            <Container sx={{ p: 2 }}>
                                <AggregationRSTIN
                                    ...
                                    inLatest={current_in}
                                    maxRTime={timeMinuteString(
                                        new Date(currentRMetrics.timeOfMax),
                                    )}
                                    maxSTime={timeMinuteString(
                                        new Date(currentSMetrics.timeOfMax),
                                    )}
                                    maxTTime={timeMinuteString(
                                        new Date(currentTMetrics.timeOfMax),
                                    )}
                                    maxInTime={timeMinuteString(
                                        new Date(currentInMetrics.timeOfMax),
                                    )}
                                    minRTime={timeMinuteString(
                                        new Date(currentRMetrics.timeOfMin),
                                    )}
                                    minSTime={timeMinuteString(
                                        new Date(currentSMetrics.timeOfMin),
                                    )}
                                    minTTime={timeMinuteString(
                                        new Date(currentTMetrics.timeOfMin),
                                    )}
                                    minInTime={timeMinuteString(
                                        new Date(currentInMetrics.timeOfMin),
                                    )}
                                />
                            </Container>
                        </Box>
