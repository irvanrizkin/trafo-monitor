...
export default function ChartPQSPF({
    ...
    const metricAvgPower = rstLineChartString({
        labels: powers.map((power) =>
            timeMinuteString(new Date(power.created_at)),
        ),
    ...
    const metricAvgReactivePower = rstLineChartString({
        labels: reactivePowers.map((reactivePower) =>
            timeMinuteString(new Date(reactivePower.created_at)),
        ),
        rData: reactivePowers.map(
            (reactivePower) => reactivePower.reactive_power_r,
        ),
        sData: reactivePowers.map(
            (reactivePower) => reactivePower.reactive_power_s,
        ),
        tData: reactivePowers.map(
            (reactivePower) => reactivePower.reactive_power_t,
        ),
    });
    const {
        reactive_power_r = 0,
        reactive_power_s = 0,
        reactive_power_t = 0,
    } = reactivePowers[reactivePowers.length - 1] || {};

    const metricAvgApparentPower = rstLineChartString({
        labels: apparentPowers.map((apparentPower) =>
            timeMinuteString(new Date(apparentPower.created_at)),
        ),
        rData: apparentPowers.map(
            (apparentPower) => apparentPower.apparent_power_r,
        ),
        sData: apparentPowers.map(
            (apparentPower) => apparentPower.apparent_power_s,
        ),
        tData: apparentPowers.map(
            (apparentPower) => apparentPower.apparent_power_t,
        ),
    });
    const {
        apparent_power_r = 0,
        apparent_power_s = 0,
        apparent_power_t = 0,
    } = apparentPowers[apparentPowers.length - 1] || {};

    const metricAvgPowerFactor = rstLineChartString({
        labels: powerFactors.map((powerFactor) =>
            timeMinuteString(new Date(powerFactor.created_at)),
        ),
    ...
    const {
        ...
        power_factor_s = 0,
        power_factor_t = 0,
    } = powerFactors[powerFactors.length - 1] || {};
                            ...
                            <Container sx={{ p: 2 }}>
                                <AggregationRST
                                    ...
                                    maxRTime={timeMinuteString(
                                        new Date(powerRMetrics.timeOfMax),
                                    )}
                                    maxSTime={timeMinuteString(
                                        new Date(powerSMetrics.timeOfMax),
                                    )}
                                    maxTTime={timeMinuteString(
                                        new Date(powerTMetrics.timeOfMax),
                                    )}
                                    minRTime={timeMinuteString(
                                        new Date(powerRMetrics.timeOfMin),
                                    )}
                                    minSTime={timeMinuteString(
                                        new Date(powerSMetrics.timeOfMin),
                                    )}
                                    minTTime={timeMinuteString(
                                        new Date(powerTMetrics.timeOfMin),
                                    )}
                                />
                            </Container>
                        </Box>
                        ...
                        >
                            ...
                            <Container sx={{ p: 2 }}>
                                <AggregationRST
                                    ...
                                    maxRTime={timeMinuteString(
                                        new Date(
                                            reactivePowerRMetrics.timeOfMax,
                                        ),
                                    )}
                                    maxSTime={timeMinuteString(
                                        new Date(
                                            reactivePowerSMetrics.timeOfMax,
                                        ),
                                    )}
                                    maxTTime={timeMinuteString(
                                        new Date(
                                            reactivePowerTMetrics.timeOfMax,
                                        ),
                                    )}
                                    minRTime={timeMinuteString(
                                        new Date(
                                            reactivePowerRMetrics.timeOfMin,
                                        ),
                                    )}
                                    minSTime={timeMinuteString(
                                        new Date(
                                            reactivePowerSMetrics.timeOfMin,
                                        ),
                                    )}
                                    minTTime={timeMinuteString(
                                        new Date(
                                            reactivePowerTMetrics.timeOfMin,
                                        ),
                                    )}
                                />
                            </Container>
                        ...
                        >
                            ...
                            <Container sx={{ p: 2 }}>
                                <AggregationRST
                                    ...
                                    maxRTime={timeMinuteString(
                                        new Date(
                                            apparentPowerRMetrics.timeOfMax,
                                        ),
                                    )}
                                    maxSTime={timeMinuteString(
                                        new Date(
                                            apparentPowerSMetrics.timeOfMax,
                                        ),
                                    )}
                                    maxTTime={timeMinuteString(
                                        new Date(
                                            apparentPowerTMetrics.timeOfMax,
                                        ),
                                    )}
                                    minRTime={timeMinuteString(
                                        new Date(
                                            apparentPowerRMetrics.timeOfMin,
                                        ),
                                    )}
                                    minSTime={timeMinuteString(
                                        new Date(
                                            apparentPowerSMetrics.timeOfMin,
                                        ),
                                    )}
                                    minTTime={timeMinuteString(
                                        new Date(
                                            apparentPowerTMetrics.timeOfMin,
                                        ),
                                    )}
                                />
                            </Container>
                        ...
                        >
                            ...
                            <Container sx={{ p: 2 }}>
                                <AggregationRST
                                    ...
                                    maxRTime={timeMinuteString(
                                        new Date(powerFactorRMetrics.timeOfMax),
                                    )}
                                    maxSTime={timeMinuteString(
                                        new Date(powerFactorSMetrics.timeOfMax),
                                    )}
                                    maxTTime={timeMinuteString(
                                        new Date(powerFactorTMetrics.timeOfMax),
                                    )}
                                    minRTime={timeMinuteString(
                                        new Date(powerFactorRMetrics.timeOfMin),
                                    )}
                                    minSTime={timeMinuteString(
                                        new Date(powerFactorSMetrics.timeOfMin),
                                    )}
                                    minTTime={timeMinuteString(
                                        new Date(powerFactorTMetrics.timeOfMin),
                                    )}
                                />
                            </Container>
                        </Box>
